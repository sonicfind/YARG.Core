...
namespace YARG.Core.IO
{
    ...
    public static class YARGChartFileReader
    {
            ...
            where TChar : unmanaged, IEquatable<TChar>, IConvertible
        {
            ...
            {
                int c = container.Get() | CharacterExtensions.ASCII_LOWERCASE_FLAG;
                if (c < 'a' || 'z' < c)
                {
                    break;
                }
            ...
            {
                int index = 0;
                while (index < length && index < descriptor.Length && container.GetBuffer()[start + index].ToInt32(null) == descriptor[index])
                {
                    ++index;
                }
