...
namespace YARG.Core.IO
{
    ...
    /// </summary>
    public struct SngFile : IDisposable, IEnumerable<KeyValuePair<string, SngFileListing>>
    {
        ...
        public readonly FixedArray<byte> LoadAllBytes(string filename)
        {
            return _listings.TryGetValue(filename, out var listing)
                ? LoadAllBytes(listing)
        ...
        public readonly FixedArray<byte> LoadAllBytes(in SngFileListing listing)
        {
            ...
            unsafe
            {
                SngFileStream.DecryptVectorized(data.Ptr, _tracker.Mask.Ptr, data.Ptr + listing.Length);
            }
            return data;
        }

        public readonly SngFileStream? CreateStream(string filename)
        {
            return _listings.TryGetValue(filename, out var listing)
                ? new SngFileStream(filename, in listing, _tracker)
                : null;
        }
        ...
        public static SngFile TryLoadFromFile(string filename, bool loadMetadata)
        {
            ...
            SngFile sng = new()
            {
                _version = filestream.Read<uint>(Endianness.Little)
            };
        ...
        private static void LoadMetadata(IniModifierCollection modifiers, Stream stream)
        {
            ...
            unsafe
            {
                container = new YARGTextContainer<byte>(&bytes, null!);
            }
        ...
        public IEnumerator<KeyValuePair<string, SngFileListing>> GetEnumerator()
        {
            return ((IEnumerable<KeyValuePair<string, SngFileListing>>) _listings).GetEnumerator();
        }

        IEnumerator IEnumerable.GetEnumerator()
        {
            return ((IEnumerable) _listings).GetEnumerator();
        }
    }
}
