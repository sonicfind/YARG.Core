...
namespace YARG.Core.NewLoading
{
    public class TrackCacheCollection : IDisposable
    {
        ...
        private readonly Dictionary<InstrumentSelection, DrumTrackCache> _drums;
        ...
        public TrackCacheCollection(YARGChart chart)
        {
            ...
            _drums = new Dictionary<InstrumentSelection, DrumTrackCache>();
        }
        ...
        public DrumTrackCache GetDrumTrack(in InstrumentSelection selection)
        {
            if (!_drums.TryGetValue(selection, out var drumCache))
            {
                if (selection.Instrument is Instrument.ProDrums or Instrument.FourLaneDrums)
                {
                    drumCache = _chart.Has(Instrument.FourLaneDrums)
                        ? DrumTrackCache.Create(_chart, _chart.FourLaneDrums, _endTime, selection)
                        : DrumTrackCache.Create(_chart, _chart.FiveLaneDrums, _endTime, selection);
                }
