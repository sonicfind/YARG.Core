...
namespace YARG.Core.NewLoading.Drums
{
    ...
    public static class DrumPlayer
    {
        ...
        public static unsafe InstrumentPlayer<Note, FourLaneSubNote> LoadFourLane(InstrumentTrack2<DifficultyTrack2<FourLaneDrums>> track, SyncTrack2 sync, YargProfile profile)
        {
            ...
            var overdrives = AllocatedArray<OverdrivePhrase>.Alloc(overdriveRanges.Count);
            var soloes = AllocatedArray<SoloPhrase>.Alloc(soloRanges.Count);
            ...
            while (curr < end)
            {
                ...
                for (int i = 0; i < FOURLANECOUNT; ++i)
                {
                    if (lanes[i].IsActive() && (i >= SNARE || (!disableKick && (isExpertPlus || !curr->Value.IsDoubleBass))))
                    {
                        ...
                        buffer[laneCount++] = new FourLaneSubNote(index, state, dynamics, DualTime.Truncate(lanes[i]) + curr->Key);
                    }
                }
        ...
        public static unsafe InstrumentPlayer<Note, FourLaneSubNote> LoadFourLane(InstrumentTrack2<DifficultyTrack2<FiveLaneDrums>> track, SyncTrack2 sync, YargProfile profile)
        {
            var diff = track[profile.CurrentDifficulty];
            Debug.Assert(diff != null, "This function should only be used with a valid difficulty");
            Debug.Assert(diff.Notes.Count > 0, "This function should only be used when notes are present");
        ...
        public static unsafe InstrumentPlayer<Note, FiveLaneSubNote> LoadFiveLane(InstrumentTrack2<DifficultyTrack2<FiveLaneDrums>> track, SyncTrack2 sync, YargProfile profile)
        {
            var diff = track[profile.CurrentDifficulty];
            Debug.Assert(diff != null, "This function should only be used with a valid difficulty");
            Debug.Assert(diff.Notes.Count > 0, "This function should only be used when notes are present");
            ...
            while (curr < end)
            {
                ...
                for (int i = 0; i < FIVELANECOUNT; ++i)
                {
                    if (lanes[i].IsActive() && (i >= SNARE || (!disableKick && (isExpertPlus || !curr->Value.IsDoubleBass))))
                    {
                        ...
                        var dynamics = i >= SNARE ? (&curr->Value.Dynamics_Snare)[i - SNARE] : DrumDynamics.None;
                        buffer[laneCount++] = new FiveLaneSubNote(index, dynamics, DualTime.Truncate(lanes[i]) + curr->Key);
                    }
                }
        ...
        public static unsafe InstrumentPlayer<Note, FiveLaneSubNote> LoadFiveLane(InstrumentTrack2<DifficultyTrack2<FourLaneDrums>> track, SyncTrack2 sync, YargProfile profile)
        {
