...
namespace YARG.Core.Song
{
    public abstract partial class SongEntry
    {
        ...
        {
            var midiFile = new YARGMidiFile(file);
            ...
            foreach (var track in midiFile)
            {
                ...
                if (!YARGMidiTrack.TRACKNAMES.TryGetValue(Encoding.ASCII.GetString(trackname), out var type))
                {
                    continue;
                }
        ...
        protected static void FinalizeDrums(ref AvailableParts parts, DrumsType drumsType)
        {
            ...
            else if (drumsType == DrumsType.ProDrums)
            {
